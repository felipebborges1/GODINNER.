-- Avoid a PL/pgSQL variable/column-name collision while reading requested photos.
...
begin
  ...
  select coalesce(array_agg(requested_photo ->> 'storage_path'), '{}'::text[])
  into requested_paths
  from jsonb_array_elements(p_photos) requested_photo;
  if exists (
    select 1 from unnest(requested_paths) path
    where path is null or length(trim(path)) = 0
      or (path not like auth.uid()::text || '/%'
        and not exists (
          select 1 from public.review_photos existing
          where existing.review_id = target.id and existing.storage_path = path
        ))
  ) then raise exception 'invalid_photo_path'; end if;

  select coalesce(array_agg(existing.storage_path), '{}'::text[])
  into removed_paths
  ...
  set rating = null,
      rating_method = 'dimensions',
      food_rating = p_food_rating,
      service_rating = p_service_rating,
      ambience_rating = p_ambience_rating,
      comment = trim(p_comment),
      amount_per_person = p_amount_per_person,
  ...
  for photo_entry in select * from jsonb_array_elements(p_photos) loop
    ...
    values (target.id, photo_entry ->> 'storage_path', (photo_entry ->> 'position')::smallint);
  end loop;
  return next;
end;
$$;
