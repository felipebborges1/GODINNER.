...
export function SearchExplorer({ aiSearchEnabled = false }: { aiSearchEnabled?: boolean }) {
  ...
  const externalResultsRef = useRef<HTMLElement>(null);
  ...
  const canRevealExternalResults = shouldSearchExternal && !externalError && (isExternalLoading || visibleExternalPlaces.length > 0);
  const revealExternalResults = () => externalResultsRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
