...
import { selectDiscoverFallback } from "@/lib/data/discover-selection";
...
export default function DiscoverPage() {
  ...
  const nearby = useMemo(() => position
    ? restaurants
      ...
      .slice(0, 6)
    : selectDiscoverFallback(restaurants), [position, restaurants]);
  ...
  const searchCatalog = useSearchCatalog(hasSearch);
  const homeSearchResults = useMemo(() => {
    const eligibleRestaurants = searchCatalog.restaurants.filter((restaurant) => restaurant.status !== "rejected" && (restaurant.status !== "pending_review" || restaurant.submittedBy === currentUserId));
    return filterRestaurants(eligibleRestaurants, { q: searchQuery }, lists, currentUserId, reviews);
  }, [currentUserId, lists, searchCatalog.restaurants, reviews, searchQuery]);
  useEffect(() => {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 lg:py-10">
    ...
    <DeferredContent label="Carregando restaurantes próximos">{position && !nearby.length ? <section className="mt-10"><EmptyState title="Ainda não temos lugares próximos de você." message="Explore o catálogo GODINNER enquanto chegamos à sua região." actionLabel="Explorar catálogo" actionHref="/search"/></section> : <DiscoverSection title="Perto de você" href="/search" restaurants={nearby} distances={position ? nearby.map((restaurant) => `${restaurant.distanceKm.toLocaleString("pt-BR", { maximumFractionDigits: 1 })} km`) : ["0,5 km", "0,9 km", "1,2 km", "1,7 km", "2,1 km", "2,4 km"]} friendCounts={friendCounts} prioritizeFirst/>}</DeferredContent>
