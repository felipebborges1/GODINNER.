...
export function AdminRestaurants({ pendingOnly = false }: { pendingOnly?: boolean }) {
  ...
  const quick = async (id: string, action: "approve" | "reject") => { if (!window.confirm(action === "approve" ? "Aprovar este restaurante?" : "Rejeitar este restaurante?")) return; const result = action === "approve" ? await approveRestaurant(id) : rejectRestaurant(id, "dados insuficientes"); showToast(result.ok ? "Moderação atualizada" : result.error); };
  return <AdminShell active={pendingOnly ? "/admin/restaurants/pending" : "/admin/restaurants"}>
    ...
    {rows.length ? <div className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm"><div className="hidden grid-cols-[64px_1.4fr_1fr_1fr_100px] gap-3 border-b p-4 text-xs font-bold uppercase text-stone-500 md:grid"><span>Foto</span><span>Restaurante</span><span>Local</span><span>Status / envio</span><span>Reviews</span></div>{rows.map((restaurant) => <div key={restaurant.id} className="grid gap-3 border-b p-4 md:grid-cols-[64px_1.4fr_1fr_1fr_100px] md:items-center"><Image src={restaurant.coverPhoto.url} alt="" width={48} height={48} className="h-12 w-12 rounded-xl object-cover"/><span><Link className="font-bold hover:text-orange-600" href={`/admin/restaurants/${restaurant.id}`}>{restaurant.name}</Link><small className="block text-stone-500">{restaurant.category === "bar" ? "Bar" : "Restaurante"} · {restaurant.cuisine.join(", ")}</small></span><span className="text-sm">{restaurant.neighborhood}<small className="block text-stone-500">{restaurant.city}</small></span><span className="text-sm"><b>{restaurant.status ?? "published"}</b><small className="block text-stone-500">{restaurant.submittedBy ? "Autoria na auditoria" : "Autoria não registrada"}{restaurant.submittedAt ? ` · ${new Date(restaurant.submittedAt).toLocaleDateString("pt-BR")}` : ""}</small></span><span className="flex items-center gap-2 text-sm">{reviews.filter((review) => review.restaurantId === restaurant.id).length}{restaurant.status === "pending_review" && <><button onClick={() => quick(restaurant.id, "approve")} className="rounded-lg bg-stone-950 px-2 py-1 text-xs font-bold text-white">Aprovar</button><button onClick={() => quick(restaurant.id, "reject")} className="rounded-lg bg-stone-200 px-2 py-1 text-xs font-bold">Rejeitar</button></>}</span></div>)}</div> : <p className="mt-8 rounded-2xl bg-white p-6 text-stone-500">Nenhum restaurante corresponde aos filtros.</p>}
  </AdminShell>;
}
