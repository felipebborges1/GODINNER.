...
function ListFormContent({ list, restaurantId, onClose, onSaved }: Omit<Props, "open">) {
  ...
  const save = async () => {
    ...
    if (list) { if (await updateList(list.id, draft)) { showToast("Lista atualizada"); onClose(); } else setError("Não conseguimos atualizar a lista agora."); return; }
    const created = await createList(draft, restaurantId);
    if (created) { trackEvent("list_created"); showToast("Lista criada"); onSaved?.(created); onClose(); } else setError("Não conseguimos criar a lista agora.");
  };
