...
export function ListDetailClient({ id }: { id: string }) {
  ...
  const remove = async (restaurantId: string) => { if (await removeRestaurantFromList(list.id, restaurantId)) showToast("Removido da lista"); };
  const destroy = async () => { if (await deleteList(list.id)) { showToast("Lista excluida"); router.push("/lists"); } };
  const cover = listRestaurants[0]?.coverPhoto.url ?? list.coverPhoto;
  return <div className="mx-auto max-w-6xl px-4 py-5 pb-28 sm:px-6 lg:py-10"><div className="relative h-52 overflow-hidden rounded-[2rem] bg-stone-100 sm:h-72"><Image src={cover} alt={`Capa da lista ${list.name}`} fill sizes="(min-width: 1024px) 1152px, 100vw" className="object-cover"/><div className="absolute inset-0 bg-gradient-to-t from-stone-950/75 to-transparent"/><div className="absolute bottom-0 p-6 text-white"><span className="inline-flex items-center gap-1 rounded-full bg-white/20 px-2 py-1 text-xs font-bold backdrop-blur">{list.isPublic ? "Pública" : <><Lock size={13}/> Privada</>}</span><h1 className="mt-3 text-3xl font-black">{list.name}</h1>{list.description && <p className="mt-1 text-sm text-stone-200">{list.description}</p>}</div></div><div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-semibold text-stone-500">{listRestaurants.length} {listRestaurants.length === 1 ? "lugar" : "lugares"}</p><div className="flex gap-2">{list.isPublic && <button type="button" onClick={share} className="inline-flex min-h-10 items-center gap-2 rounded-full bg-stone-100 px-3 text-sm font-bold"><Share2 size={16}/> Compartilhar</button>}{canManage && <button type="button" onClick={() => setEditOpen(true)} aria-label="Editar lista" className="grid min-h-10 min-w-10 place-items-center rounded-full bg-stone-100"><Edit3 size={17}/></button>}{canManage && <button type="button" onClick={() => setDeleteOpen(true)} aria-label="Excluir lista" className="grid min-h-10 min-w-10 place-items-center rounded-full bg-red-50 text-red-600"><Trash2 size={17}/></button>}</div></div><div className="mt-7 flex gap-2 rounded-2xl bg-stone-100 p-1 w-fit"><button type="button" onClick={() => setView("list")} className={`rounded-xl px-4 py-2 text-sm font-bold ${view === "list" ? "bg-white shadow-sm" : "text-stone-500"}`}>Lista</button><button type="button" onClick={() => setView("map")} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold ${view === "map" ? "bg-white shadow-sm" : "text-stone-500"}`}><Map size={16}/>Mapa</button></div>{!listRestaurants.length ? <div className="mt-6"><EmptyState title="Esta lista ainda está vazia" message="Adicione restaurantes a partir do perfil de cada lugar." actionLabel={owner ? "Adicionar restaurante" : "Explorar restaurantes"} actionHref="/search"/></div> : view === "map" ? <div className="mt-6"><MapView restaurants={listRestaurants}/></div> : <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{listRestaurants.map((restaurant) => <div key={restaurant.id} className="relative"><RestaurantCard restaurant={restaurant}/>{owner && <button type="button" onClick={() => void remove(restaurant.id)} className="absolute right-3 top-3 rounded-full bg-white/95 px-3 py-2 text-xs font-black text-red-600 shadow" aria-label={`Remover ${restaurant.name} da lista`}>Remover</button>}</div>)}</div>}<ListFormSheet open={editOpen} onClose={() => setEditOpen(false)} list={list}/><Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Excluir lista"><p className="text-sm leading-6 text-stone-600">Esta ação remove a lista personalizada, mas não apaga os restaurantes salvos em outras listas.</p><div className="mt-6 flex gap-3"><button type="button" onClick={() => setDeleteOpen(false)} className="flex-1 rounded-2xl bg-stone-100 py-3 text-sm font-bold">Cancelar</button><button type="button" onClick={() => void destroy()} className="flex-1 rounded-2xl bg-red-600 py-3 text-sm font-bold text-white">Excluir</button></div></Modal></div>;
}
