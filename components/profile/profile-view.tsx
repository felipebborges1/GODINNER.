...
export function ProfileView({ userId, own }: { userId: string; own: boolean }) {
  ...
  const follow = async () => {
    ...
    const added = await toggleFollow(user.id);
    trackEvent("user_followed", { userId: user.id, following: added });
    showToast(added ? `Seguindo ${user.name}` : `Você deixou de seguir ${user.name}`);
  };
