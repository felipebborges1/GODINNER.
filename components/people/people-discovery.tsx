...
export function PeopleDiscovery() {
  ...
  const profilesWithoutActivities = useMemo(() => {
    const normalizedQuery = normalize(query);
    if (!normalizedQuery) return [];
    const activityUserIds = new Set(activities.map(({ user }) => user.id));
    return users
      .filter((user) => user.id !== currentUserId && !activityUserIds.has(user.id))
      .filter((user) => normalize([user.name, user.username, user.neighborhood].join(" ")).includes(normalizedQuery))
      .sort((a, b) => Number(followingIds.has(a.id)) - Number(followingIds.has(b.id)));
  }, [activities, currentUserId, followingIds, query, users]);
  ...
  return <main className="mx-auto max-w-3xl px-4 py-6 pb-28 sm:px-6 lg:py-12">
    ...
    {activities.length || profilesWithoutActivities.length ? <div className="mt-8 grid gap-4 md:grid-cols-2">{activities.slice(0, 12).map(({ review, user, restaurant }, index) => <article key={review.id} className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-stone-100"><div className="flex items-center gap-3 p-4"><Link href={`/user/${user.username}`} aria-label={`Ver perfil de ${user.name}`}><UserAvatar src={user.avatar} name={user.name}/></Link><div className="min-w-0 flex-1"><Link href={`/user/${user.username}`} className="block truncate text-sm font-black">{user.name}</Link><p className="mt-1 text-xs text-stone-500">esteve no <Link href={`/restaurant/${restaurant.slug}`} className="font-bold text-stone-700">{restaurant.name}</Link></p></div><Button variant={isFollowing(user.id) ? "soft" : "accent"} onClick={() => void follow(user.id, user.name)} aria-label={`${isFollowing(user.id) ? "Deixar de seguir" : "Seguir"} ${user.name}`}>{isFollowing(user.id) ? "Seguindo" : "Seguir"}</Button></div><div className="relative"><ReviewMedia photos={review.photos} alt={`Experiência de ${user.name} em ${restaurant.name}`} rounded={false} priority={index === 0} fallback={<Link href={`/restaurant/${restaurant.slug}`} className="relative block aspect-[4/3] overflow-hidden">{restaurant.hasGooglePlaceCover ? <GooglePlaceCover slug={restaurant.slug} alt={restaurant.name} variant="card" priority={index === 0}/> : restaurant.coverPhoto.url ? <Image src={restaurant.coverPhoto.url} alt={restaurant.name} fill priority={index === 0} loading={index === 0 ? "eager" : "lazy"} sizes="(min-width: 768px) 360px, 100vw" className="object-cover"/> : <RestaurantPhotoUnavailable alt={restaurant.name} variant="card"/>}</Link>}/><div className="pointer-events-none absolute bottom-3 left-3"><RatingBadge rating={review.rating}/></div></div><div className="p-4"><p className="text-sm leading-6 text-stone-700">{review.comment || "Compartilhou uma experiência neste lugar."}</p><p className="mt-2 text-xs text-stone-500">{restaurant.cuisine[0]} · {restaurant.neighborhood}</p></div></article>)}{profilesWithoutActivities.map((user) => <article key={user.id} className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-stone-100"><div className="flex items-center gap-3"><Link href={`/user/${user.username}`} aria-label={`Ver perfil de ${user.name}`}><UserAvatar src={user.avatar} name={user.name}/></Link><div className="min-w-0 flex-1"><Link href={`/user/${user.username}`} className="block truncate text-sm font-black">{user.name}</Link><p className="mt-1 truncate text-xs text-stone-500">@{user.username} · {user.neighborhood}</p></div><Button variant={isFollowing(user.id) ? "soft" : "accent"} onClick={() => void follow(user.id, user.name)} aria-label={`${isFollowing(user.id) ? "Deixar de seguir" : "Seguir"} ${user.name}`}>{isFollowing(user.id) ? "Seguindo" : "Seguir"}</Button></div><p className="mt-5 text-sm leading-6 text-stone-600">Ainda não publicou experiências. Siga para acompanhar quando compartilhar novos lugares.</p></article>)}</div> : <div className="mt-8"><p className="rounded-3xl border border-dashed border-stone-300 bg-stone-50 px-6 py-12 text-center text-sm text-stone-600">{query ? `Nenhum perfil encontrado para “${query}”.` : "Ainda não encontramos experiências de outras pessoas por aqui."}</p></div>}
