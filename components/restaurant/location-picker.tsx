...
type LocationMapsApi = { Map: new (element: HTMLElement, options: Record<string, unknown>) => LocationMapInstance; Marker: new (options: Record<string, unknown>) => LocationMarkerInstance; Geocoder: new () => { geocode: (request: { location: LatLng }, callback: (results: GoogleGeocoderResult[] | null, status: string) => void) => void } };
...
export function LocationPicker({ value, onChange, onAddressResolved }: { value?: RestaurantCoordinates; onChange: (coordinates: RestaurantCoordinates) => void; onAddressResolved: (value: { address: string; city?: string; neighborhood?: string; region?: string; country?: string }) => void }) {
  ...
  const resolveAddress = useCallback((coordinates: RestaurantCoordinates) => {
    ...
    new maps.Geocoder().geocode({ location: { lat: coordinates.latitude, lng: coordinates.longitude } }, (results, status) => {
      if (status === "OK" && results?.[0]) { onAddressResolved(parseRestaurantAddress(results[0])); setMessage("Endereço, bairro e cidade foram preenchidos. Confira os dados antes de continuar."); }
