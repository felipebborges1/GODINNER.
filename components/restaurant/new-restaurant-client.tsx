...
type NewRestaurantClientProps = {
  /** Used by the unified review journey; the legacy /restaurant/new query remains supported. */
  initialPlace?: GooglePlaceCandidate;
  initialQuery?: string;
  startManual?: boolean;
  onBack?: () => void;
};
...
export function NewRestaurantClient({ initialPlace: placeFromReviewFlow, initialQuery: initialQueryFromFlow, startManual = false, onBack }: NewRestaurantClientProps) {
  const params = useSearchParams();
  const initialSelectedPlace = placeFromReviewFlow ?? selectedPlaceFromParams(params);
  const initialQuery = initialQueryFromFlow ?? params.get("name") ?? "";
  const { submitRestaurant, createRestaurantFromGooglePlace } = useAppContext();
  const [mode, setMode] = useState<Mode>(startManual ? "manual" : "find");
