  ...
  return <div className="pb-28 lg:pb-12">
    ...
    <main className="mx-auto max-w-6xl px-4 pt-6 sm:px-6 lg:pt-9">
      <div className="lg:grid lg:grid-cols-[1fr_360px] lg:gap-12">
        ...
        <section className="mt-6 grid grid-cols-2 gap-3 lg:mt-0"><div className="rounded-3xl bg-stone-950 p-5 text-white"><p className="text-xs font-black uppercase tracking-wide text-stone-300">GODINNER</p><p className="mt-1 text-3xl font-black">{formatRating(rating)}</p><p className="mt-1 text-xs text-stone-300">{rating !== null ? `${restaurantReviews.length} avaliações` : "ainda sem avaliações"}</p></div><div className="rounded-3xl bg-orange-50 p-5"><p className="text-xs font-black uppercase tracking-wide text-orange-700">Seus amigos</p><p className="mt-1 text-3xl font-black">{formatRating(friendsRating)}</p><p className="mt-1 text-xs text-orange-700">{friendsRating !== null ? "nota dos amigos" : "ainda não avaliaram"}</p></div>{hasDimensionAverages && <div className="col-span-2 rounded-3xl bg-stone-50 p-4 text-xs text-stone-600"><b className="text-stone-900">Médias da comunidade</b><p className="mt-2">Comida {formatRating(dimensionAverages.food)} · Serviço {formatRating(dimensionAverages.service)} · Ambiente {formatRating(dimensionAverages.ambience)}</p></div>}</section>
      </div>
