...
import { MichelinRecognitionBadge, MichelinRecognitionSource } from "@/components/ui/michelin-recognition";
    ...
    <main className="mx-auto max-w-6xl px-4 pt-6 sm:px-6 lg:pt-9">
      <div className="lg:grid lg:grid-cols-[1fr_360px] lg:gap-12">
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">{restaurant.cuisine.map((cuisine) => <CuisineChip key={cuisine} cuisine={cuisine}/>) }<PriceBadge price={restaurant.priceRange}/>{restaurant.acceptsDuoGourmet && <DuoGourmetIndicator/>}<MichelinRecognitionBadge recognition={restaurant.michelin}/></div>
      ...
      <section className="mt-7 flex gap-3"><button onClick={handleWant} className={`grid min-h-14 min-w-14 place-items-center rounded-2xl border ${isWanted ? "border-orange-500 bg-orange-500 text-white" : "border-stone-200 bg-white"}`} aria-label={isWanted ? "Remover de Quero conhecer" : "Adicionar a Quero conhecer"}><Heart size={21} fill={isWanted ? "currentColor" : "none"}/></button><Link href={`/review/new?restaurant=${restaurant.slug}`} className="flex min-h-14 flex-1 items-center justify-center gap-2 rounded-2xl bg-stone-950 text-sm font-black text-white"><Check size={19}/> Já fui / Avaliar</Link><button onClick={() => setOptionsOpen(true)} className="grid min-h-14 min-w-14 place-items-center rounded-2xl bg-stone-100" aria-label="Mais opções"><Ellipsis size={23}/></button></section>
        ...
        <aside className="space-y-5">{restaurant.acceptsDuoGourmet && <section className="rounded-3xl border border-orange-100 bg-orange-50 p-5"><DuoGourmetIndicator/><p className="mt-3 text-xs text-stone-600">Benefício sujeito às regras e disponibilidade do Duo Gourmet.</p></section>}<MichelinRecognitionSource recognition={restaurant.michelin}/>{restaurant.priceRange && <section className="rounded-3xl bg-orange-50 p-5"><p className="text-sm font-black">Faixa editorial</p><p className="mt-2 text-2xl font-black">{restaurant.priceRange}</p><p className="mt-1 text-xs text-stone-500">Referência inicial do catálogo</p></section>}<section className="rounded-3xl border border-stone-100 p-5"><h2 className="text-lg font-black">Sobre</h2><p className="mt-3 flex items-start gap-2 text-sm text-stone-600"><MapPin size={17} className="mt-0.5 shrink-0"/>{restaurant.address}</p></section><section><h2 className="mb-3 text-lg font-black">Localização</h2><DeferredMapView restaurants={[restaurant]}/></section></aside>
      </div>
    </main>
