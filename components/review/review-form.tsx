...
export function ReviewForm({ restaurant }: { restaurant: Restaurant }) {
  ...
  const [recommendationsUnlocked, setRecommendationsUnlocked] = useState(false);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (publishedReview.recommendationsUnlocked && !unlockTracked.current) {
      unlockTracked.current = true;
      trackEvent("recommendation_unlocked", { review_count: 3, maturity_state: "early" });
    }
