...
export function ReviewSocialActions({ reviewId }: { reviewId: string }) {
  ...
  const startReply = (target: ReviewComment) => {
    ...
    const author = users.find((user) => user.id === target.userId);
    const username = author?.username ?? "usuario";
    setExpandedRoots((current) => new Set(current).add(target.parentCommentId ?? target.id));
    setReplyTarget(target);
    setComment(`@${username} `);
    setCursor(username.length + 2);
  ...
  const renderComment = (item: ReviewComment, isReply = false) => {
    ...
    return <div id={`review-comment-${item.id}`} key={item.id} className={`flex gap-2 rounded-2xl bg-stone-50 p-3 ${isReply ? "ml-4 border-l-2 border-orange-100" : ""}`}><UserAvatar src={author?.avatar ?? null} name={author?.name ?? "Usuário"}/><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black">{author?.name ?? "Usuário"}</p><div className="flex shrink-0 items-center gap-1"><button type="button" onClick={() => startReply(item)} className="min-h-8 rounded-full px-2 text-xs font-black text-orange-600 hover:bg-white hover:text-orange-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">Responder</button>{canManageReviewComment(item, currentUserId, isAdmin) && <button type="button" aria-label="Excluir comentário" onClick={() => void deleteReviewComment(reviewId, item.id)} className="grid h-8 w-8 place-items-center rounded-full text-stone-400 hover:bg-white hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"><Trash2 size={15}/></button>}</div></div><p className="mt-1 whitespace-pre-wrap break-words text-sm leading-5 text-stone-700">{commentSegments(item.body, item.mentions).map((segment, index) => typeof segment === "string" ? <span key={`${item.id}-text-${index}`}>{segment}</span> : <Link key={`${item.id}-mention-${segment.userId}-${index}`} href={`/user/${segment.username}`} className="font-bold text-orange-600 hover:text-orange-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500">@{segment.username}</Link>)}</p></div></div>;
  };
