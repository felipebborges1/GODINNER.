...
export function ReviewSocialActions({ reviewId }: { reviewId: string }) {
  ...
  return <div className="mt-4 border-t border-stone-100 pt-3">
    <div className="flex items-center gap-1">
      ...
      <button type="button" onClick={() => void openComments()} aria-expanded={commentsOpen} aria-controls={`review-comments-${reviewId}`} className="inline-flex min-h-11 items-center gap-2 rounded-full px-3 text-sm font-bold text-stone-600 transition hover:bg-stone-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"><MessageCircle size={18}/><span>{summary.commentCount ? `${summary.commentCount} ${summary.commentCount === 1 ? "comentário" : "comentários"}` : "Comentar"}</span></button>
    </div>
