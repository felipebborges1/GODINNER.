...
test('Discover context remains legacy read; both Search entry points opt in',()=>{const context=fs.readFileSync(new URL('../context/app-context.tsx',import.meta.url),'utf8');const home=fs.readFileSync(new URL('../components/discover/discover-page.tsx',import.meta.url),'utf8');const explorer=fs.readFileSync(new URL('../components/search/search-explorer.tsx',import.meta.url),'utf8');assert.ok(!context.includes('loadSearchCatalog('));assert.ok(!context.includes('loadVisibleCatalog('));assert.ok(context.includes('client.from("restaurants").select("*").order("created_at", { ascending: false })'));assert.ok(home.includes('useSearchCatalog(hasSearch)'));assert.ok(home.includes('selectDiscoverFallback(restaurants)'));assert.ok(explorer.includes('useSearchCatalog(true)'));});
