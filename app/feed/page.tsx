...
export default function FeedPage() {
  ...
  return <div className="mx-auto max-w-xl px-4 py-5 pb-28 lg:py-10"><div className="flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-orange-600">SEGUINDO</p><h1 className="text-3xl font-black tracking-tight">Experiências recentes</h1></div><Button href="/people" variant="soft" className="shrink-0">Encontrar pessoas</Button></div>{activities.length ? <div className="mt-6 grid gap-4">{activities.map((review) => { const user = users.find((item) => item.id === review.userId); return user ? <ReviewCard key={review.id} review={review} user={user} restaurant={restaurants.find((restaurant) => restaurant.id === review.restaurantId)}/> : null; })}</div> : <div className="mt-6"><EmptyState title="Seu feed está com fome." message="Siga pessoas para descobrir onde elas estão comendo." actionLabel="Encontrar pessoas" actionHref="/people"/></div>}</div>;
}
