...
export default function DiscoverPage() {
  ...
  const { currentUserId, follows, restaurants, reviews, users, isLoading, dataError, retryData } = useAppContext();
  ...
  return <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 lg:py-10">
    ...
    <section className="pt-9 lg:pt-0"><button onClick={() => setIsLocationOpen(true)} className="inline-flex items-center gap-1 text-sm font-semibold text-stone-500"><MapPin size={16} className="text-orange-500"/>{location}<ChevronDown size={15}/></button><h1 className="mt-3 max-w-xl text-4xl font-black tracking-[-0.06em] sm:text-5xl">Onde vamos hoje?</h1><p className="mt-3 max-w-lg text-base leading-7 text-stone-600">Descubra lugares através de pessoas em quem você confia.</p><div className="mt-6 max-w-xl"><SearchBar navigateOnFocus placeholder="Restaurante, comida, bairro ou chef"/></div><div className="-mx-4 mt-4 flex gap-2 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0">{quickFilters.map((filter) => <FilterChip key={filter.label} label={filter.label} href={filter.href}/>)}</div></section>
