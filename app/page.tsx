...
function isWithinManualState(place: GooglePlaceCandidate, region: { city: string; region?: string; countryCode?: string; scopeType?: string }) {
  if (region.scopeType !== "state") return true;
  const expectedState = region.region || region.city;
  return normalize(place.region ?? place.city ?? "") === normalize(expectedState)
    && (!region.countryCode || !place.countryCode || region.countryCode.toUpperCase() === place.countryCode.toUpperCase());
}
...
export default function DiscoverPage() {
  ...
  const { places: externalPlaces, resultQuery: externalResultQuery, isLoading: isExternalLoading, error: externalError, retryAfterSeconds: externalRetryAfterSeconds, diagnostics: externalDiagnostics, searchPlaces: searchExternalPlaces, clear: clearExternalSearch } = useGooglePlaceSearch();
  const showExternalSearchDiagnostics = typeof window !== "undefined" && (window.location.hostname === "localhost" || /^godinner-beta-[a-z0-9]+-fbb4\.vercel\.app$/.test(window.location.hostname));
  ...
  const externalSearchQuery = googleQueryForReview(searchQuery, mode === "manual" ? manualRegion ?? undefined : undefined);
  const visibleExternalPlaces = !externalError && externalResultQuery === externalSearchQuery
    ? externalPlaces.filter((place) => !manualRegion || isWithinManualState(place, manualRegion))
    : [];
  const startMapReview = () => router.push(mapReviewUrl({ query: searchQuery, manualRegion: mode === "manual" ? manualRegion ?? undefined : undefined, devicePosition: mode === "device" ? devicePosition ?? undefined : undefined }));
  ...
  const runExternalSearch = useCallback(async (query: string, attempt: number, source: "automatic" | "manual" | "retry", locationBias?: { latitude: number; longitude: number }) => {
    if (attempt !== externalSearchAttempt.current) return;
    const results = await searchExternalPlaces(query, {
      position: locationBias,
      diagnostic: {
        attemptId: `discover-external-${attempt}`,
        source,
        manualRegionType: mode === "manual" ? manualRegion?.scopeType ?? "unknown" : undefined,
      },
    });
    if (attempt !== externalSearchAttempt.current) return;
    if (results.length) trackEvent("discover_external_results_shown", { count: results.length, hasLocationBias: Boolean(locationBias) });
  }, [manualRegion?.scopeType, mode, searchExternalPlaces]);
  ...
  const searchOutsideCatalog = () => {
    ...
    void runExternalSearch(googleQueryForReview(query, mode === "manual" ? manualRegion ?? undefined : undefined), attempt, externalError ? "retry" : "manual", mode === "device" && !homeSearchGeography.explicitCity && !queryIncludesExplicitPlaceContext(query) ? devicePosition ?? undefined : undefined);
  };
  ...
  if (hasSearch && !homeSearchResults.length && !homeSearchGeography.ambiguousCities.length) return <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 lg:py-10"><header className="flex items-center justify-between lg:hidden"><Brand/><Link href="/feed" aria-label="Abrir feed" className="grid h-10 w-10 place-items-center rounded-full bg-stone-100 text-stone-800"><UsersRound size={20}/></Link></header><section className="pt-9 lg:pt-0"><h1 className="max-w-xl text-4xl font-black tracking-[-0.06em] sm:text-5xl">Onde vamos hoje?</h1><div className="mt-6 max-w-xl"><SearchBar value={searchQuery} onChange={(value) => { setSearchQuery(value); resetExternalSearch(); }} onClear={() => { setSearchQuery(""); resetExternalSearch(); }} onFocus={showLocationNudge} placeholder="Restaurante, comida, bairro ou chef"/><div className="mt-1"><ExploreLocationPicker/></div><LocationSearchNudge/></div></section><section className="mt-8 max-w-xl">{isExternalLoading && <p role="status" className="rounded-2xl bg-stone-100 p-4 text-sm font-semibold text-stone-600">Procurando lugares…</p>}{externalError && <MapReviewEntryCta failed onStart={startMapReview}/>} {!isExternalLoading && !externalError && !visibleExternalPlaces.length && <MapReviewEntryCta onStart={startMapReview}/>} {visibleExternalPlaces.length > 0 && <><p className="text-xs font-black uppercase tracking-wide text-orange-600">LUGARES ENCONTRADOS</p><h2 className="mt-1 text-xl font-black">Lugares encontrados</h2><p className="mt-1 text-xs text-stone-500">Dados fornecidos pelo Google</p><div className="mt-4 grid gap-2">{visibleExternalPlaces.map((place) => <button type="button" key={place.placeId} onClick={() => { const existing = restaurants.find((restaurant) => restaurant.googlePlaceId === place.placeId && restaurant.status === "published"); router.push(existing ? `/restaurant/${existing.slug}` : reviewNewUrl(place)); }} className="flex min-h-20 w-full items-start gap-3 rounded-2xl bg-white p-4 text-left shadow-sm ring-1 ring-stone-100"><span className="min-w-0"><b className="block break-words text-sm">{place.name}</b><span className="mt-1 block break-words text-xs text-stone-500">{place.address || [place.neighborhood, place.city, place.country].filter(Boolean).join(" · ")}</span></span></button>)}</div></>}{showExternalSearchDiagnostics && externalDiagnostics && <details className="mt-4 rounded-xl border border-dashed border-stone-300 p-3 text-xs text-stone-600"><summary className="cursor-pointer font-bold">Diagnóstico da busca externa (somente Preview)</summary><ul className="mt-2 grid gap-1"><li>{externalDiagnostics.attemptId ?? "tentativa"} · {externalDiagnostics.source ?? "desconhecida"} · região: {externalDiagnostics.manualRegionType ?? "nenhuma"}</li><li>Rota: HTTP {externalDiagnostics.routeStatus} · Google: {externalDiagnostics.upstreamStatus ?? "não informado"} · GPS enviado: {externalDiagnostics.hasPosition ? "sim" : "não"}</li><li>Duração: {externalDiagnostics.durationMs} ms{externalDiagnostics.failureCode ? ` · causa: ${externalDiagnostics.failureCode}` : ""}{externalDiagnostics.networkCode ? ` · rede: ${externalDiagnostics.networkCode}` : ""}</li></ul></details>}</section><LoginWall open={loginOpen} onClose={() => setLoginOpen(false)} next={mapReviewUrl({ query: searchQuery, manualRegion: mode === "manual" ? manualRegion ?? undefined : undefined, devicePosition: mode === "device" ? devicePosition ?? undefined : undefined })}/></div>;
