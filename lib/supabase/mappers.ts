...
export function mapReview(row: ReviewRow, photos: RestaurantPhoto[] = []): Review { return { id: row.id, userId: row.user_id, restaurantId: row.restaurant_id, rating: Number(row.rating), ratingMethod: row.rating_method, foodRating: row.food_rating, serviceRating: row.service_rating, ambienceRating: row.ambience_rating, comment: row.comment, photos, amountPerPerson: row.amount_per_person ?? undefined, currency: row.currency ?? undefined, visitDate: row.visit_date, createdAt: row.created_at, updatedAt: row.updated_at }; }
