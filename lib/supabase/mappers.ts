...
export function mapReview(row: ReviewRow, photos: RestaurantPhoto[] = []): Review { const legacyRating = row.rating === null ? null : Number(row.rating); const foodRating = row.food_rating; const serviceRating = row.service_rating; const ambienceRating = row.ambience_rating; const rating = row.rating_method === "dimensions" ? ((foodRating ?? 0) + (serviceRating ?? 0) + (ambienceRating ?? 0)) / 3 : (legacyRating ?? 0) / 2; return { id: row.id, userId: row.user_id, restaurantId: row.restaurant_id, rating, legacyRating, ratingMethod: row.rating_method, foodRating, serviceRating, ambienceRating, comment: row.comment, photos, amountPerPerson: row.amount_per_person ?? undefined, visitDate: row.visit_date, createdAt: row.created_at, updatedAt: row.updated_at }; }
export function mapReviewPhoto(row: ReviewPhotoRow, url: string): RestaurantPhoto { return { id: row.id, reviewId: row.review_id, storagePath: row.storage_path, url, alt: "Foto da experiência" }; }
